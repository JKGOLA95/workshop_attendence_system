...
async def _send_wati_template(
    *, phone: str, template_name: str, params_obj: List[dict],
    ...
    """Send a WATI template (tries v2, v1, bulk)."""
    ...
    try:
        ...
        payload = {"whatsappNumber": phone, "template_name": template_name, "broadcast_name": bname, "parameters": params_obj}
        if chan:
            payload["channel_number"] = chan
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.post(url, headers=headers, json=payload)
    ...
    try:
        ...
        payload = {"whatsappNumbers": [phone], "template_name": template_name, "broadcast_name": bname, "parameters": params_obj}
        if chan:
            payload["channel_number"] = chan
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.post(url, headers=headers, json=payload)
    ...
    try:
        url = f"{base}/api/v1/sendTemplateMessages"
        payload = {"receivers": [phone], "template_name": template_name, "broadcast_name": bname, "parameters": params_obj}
        if chan:
            payload["channel_number"] = chan
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.post(url, headers=headers, json=payload)
...
async def _update_send_status(attendee_id: str, email_status: str, wa_status: str,
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                """UPDATE attendees
                      SET qr_email_status=$2, qr_whatsapp_status=$3,
                          qr_sent_at=$4, qr_last_error=$5, updated_at=NOW()
                    WHERE id=$1""",
    ...
    async with SEND_SEMAPHORE:
        ...
        try:
            ...
            if phone and WATI_TEMPLATE_NAME_QR:
                ...
                params_obj = [{"name": "name", "value": attendee.name},
...
async def send_entry_pass(attendee_row, entry_time: datetime):
    """Send entry pass via Brevo + WATI."""
    try:
        name, email, mobile, batch = attendee_row["name"], attendee_row["email"], attendee_row["mobile"], attendee_row["batch"]

        try:
            await _send_brevo_email(
                to_email=email,
                subject=f"Welcome {name} — Workshop Entry Pass",
                text=(f"Welcome {name}!\nThis is your today's workshop pass.\n"
                      f"Date: {entry_time.date()}\nEntry time: {entry_time.strftime('%H:%M')}\nBatch: {batch}\nPlease show this to BCI staff.")
            )
        except Exception as e:
        ...
        try:
            ...
            if phone and WATI_TEMPLATE_NAME_ENTRY:
                params_obj = [{"name": "name", "value": name},
                              {"name": "batch", "value": batch},
                              {"name": "email", "value": email}]
                ok = await _send_wati_template(
                    phone=phone, template_name=WATI_TEMPLATE_NAME_ENTRY,
                    params_obj=params_obj, broadcast_name=WATI_BROADCAST_NAME
                )
...
async def scan_qr(scan_data: QRScanResult, token: str = Depends(verify_token)):
    try:
        async with db_pool.acquire() as conn:
            ...
            await conn.execute("INSERT INTO attendance (attendee_id, entry_time, created_at) VALUES ($1,$2,$3)",
...
async def get_attendance_dashboard(token: str = Depends(verify_token)):
    async with db_pool.acquire() as conn:
        ...
        batch_rows = await conn.fetch("""SELECT a.batch, COUNT(a.id) AS total_registered,
                                                COUNT(att.id) AS total_attended
                                           FROM attendees a
                                           LEFT JOIN attendance att ON a.id=att.attendee_id
                                       GROUP BY a.batch""")
    return {"total_attendees": total, "marked_attendance": attended,
...
async def upload_csv(file: UploadFile = File(...), token: str = Depends(verify_token)):
    ...
    try:
        ...
        s = content.decode("utf-8-sig")   # BOM safe
        ...
        if df.columns.tolist() != required:
            raise HTTPException(status_code=400, detail=f"CSV must contain exact columns: {required}, got {df.columns.tolist()}")

        attendees = [Attendee(name=str(r["name"]).strip(),
                              email=str(r["email"]).strip(),
                              #email=EmailStr(str(r["email"]).strip()),
...
async def resend_pending(limit: int = 200, token: str = Depends(verify_token)):
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("""SELECT id,name,email,mobile,batch,qr_code
                                     FROM attendees
                                    WHERE (qr_email_status<>'sent' OR qr_whatsapp_status<>'sent')
                                    ORDER BY created_at LIMIT $1""", limit)
    results = await asyncio.gather(*(send_qr_code(r["id"],
                                                  Attendee(name=r["name"], email=r["email"],
                                                           mobile=r["mobile"], batch=r["batch"]),
